...
def run_model(mon_, lag_mon):
    ...
    out_dir_path = '/home/users/roms/balaji/aghil/nino_seed_version/nino/SSTA_3_VATA/out/' + month_ + '/lead' + str(lag_mon)
    ...
    ohc0=ds.t_5m_300_avg.sel(lat=slice(-55,60)).sel(time=slice('1871','1980')) # in degree

    ds=xr.open_dataset('/home/users/roms/balaji/athul/enso/data/soda/soda_sst_temp_500m300.nc').drop_vars(["LEV1_25", "lev"])
    print(ds)
    ohc1=ds.temp_500m300.sel(lat=slice(-55,60)).sel(time=slice('1871','1980')) # in degree
 
    ds=xr.open_dataset('/home/users/roms/balaji/athul/enso/data/soda/soda_sst_temp_750m500.nc').drop_vars("LEV1_25")
    print(ds)
    ohc2=ds.temp_750m500.sel(lat=slice(-55,60)).sel(time=slice('1871','1980')) # in degree
    ...
    # nino_target_series 
    (ssta2, ssta2_pm1, ssta2_pm2,ohca0_2, ohca0_2_pm1, ohca0_2_pm2,
     ohca1_2, ohca1_2_pm1, ohca1_2_pm2,ohca2_2, ohca2_2_pm1, ohca2_2_pm2,
     nino_target) = modify_soda_(sst,ohc0,ohc1,ohc2,predict_mon=mon_,lag_=lag_mon)
    ...
    # --- SSTA ---
    ssta_tau_0.append(ssta2)
    ssta_tau_1.append(ssta2_pm1)
    ssta_tau_2.append(ssta2_pm2)

    # --- 0-300 ---
    ohca0_tau_0.append(ohca0_2)
    ohca0_tau_1.append(ohca0_2_pm1)
    ohca0_tau_2.append(ohca0_2_pm2)

    # --- 300-500 ---
    ohca1_tau_0.append(ohca1_2)
    ohca1_tau_1.append(ohca1_2_pm1)
    ohca1_tau_2.append(ohca1_2_pm2)

    # --- 500-750 ---
    ohca2_tau_0.append(ohca2_2)
    ohca2_tau_1.append(ohca2_2_pm1)
    ohca2_tau_2.append(ohca2_2_pm2)

    # --- Target ---
    nino_lead.append(nino_target)
    ## load CMIP data ###############
    for jj in range(tot_models):
        (ssta2, ssta2_pm1, ssta2_pm2,ohca0_2, ohca0_2_pm1, ohca0_2_pm2,
         ohca1_2, ohca1_2_pm1, ohca1_2_pm2,ohca2_2, ohca2_2_pm1, ohca2_2_pm2,
         nino_target) = select_cmip_model(model_no_=jj,predict_mon=mon_,lag_=lag_mon)
    ...
    # --- Merge 0-300 ---
    merged_ohc0_0 = xr.concat(ohca0_tau_0, dim='time')
    merged_ohc0_1 = xr.concat(ohca0_tau_1, dim='time')
    merged_ohc0_2 = xr.concat(ohca0_tau_2, dim='time')

    # --- Merge 300-500 ---
    merged_ohc1_0 = xr.concat(ohca1_tau_0, dim='time')
    merged_ohc1_1 = xr.concat(ohca1_tau_1, dim='time')
    merged_ohc1_2 = xr.concat(ohca1_tau_2, dim='time')

    # --- Merge 500-750 ---
    merged_ohc2_0 = xr.concat(ohca2_tau_0, dim='time')
    merged_ohc2_1 = xr.concat(ohca2_tau_1, dim='time')
    merged_ohc2_2 = xr.concat(ohca2_tau_2, dim='time')
    ...
    # --- Assign common time ---
    for da in (merged_sst0, merged_sst1, merged_sst2,merged_ohc0_0, merged_ohc0_1, merged_ohc0_2,
           merged_ohc1_0, merged_ohc1_1, merged_ohc1_2,merged_ohc2_0, merged_ohc2_1, merged_ohc2_2, merged_nino):
        da['time'] = new_time_sst

    im_ = np.empty([merged_sst0.shape[0],merged_sst0.shape[1],merged_sst0.shape[2],12])
    nan_ = -999
    ...
    # --- 0-300 ---
    im_[:, :, :, 3] = merged_ohc0_0.fillna(nan_)
    im_[:, :, :, 4] = merged_ohc0_1.fillna(nan_)
    im_[:, :, :, 5] = merged_ohc0_2.fillna(nan_)

    # --- 300-500 ---
    im_[:, :, :, 6] = merged_ohc1_0.fillna(nan_)
    im_[:, :, :, 7] = merged_ohc1_1.fillna(nan_)
    im_[:, :, :, 8] = merged_ohc1_2.fillna(nan_)

    # --- 500-750 ---
    im_[:, :, :, 9]  = merged_ohc2_0.fillna(nan_)
    im_[:, :, :, 10] = merged_ohc2_1.fillna(nan_)
    im_[:, :, :, 11] = merged_ohc2_2.fillna(nan_)
...
if __name__ == '__main__':
    tasks = [(m, l) for m in range(0, 4) for l in range(1, 25)]
